...
if __name__ == "__main__":
    ...
    if dist.get_rank() == 0:
        ...
        def generate(config: Config):
            ...
            dist.broadcast_object_list([[config.prompt], config.max_gen_len, config.temperature, config.top_p])

            results = generator.generate(
                [config.prompt], max_gen_len=config.max_gen_len, temperature=config.temperature, top_p=config.top_p
            )
